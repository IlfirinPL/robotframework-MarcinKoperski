...
class WindowManagerTests(unittest.TestCase):
    ...
    def test_get_window_ids(self):
        manager = WindowManager()
        browser = self._make_mock_browser({'id': 'win1', 'name': 'win1', 'title': "Title 1", 'url': 'http://localhost/page1.html'},
            {'id': 'win2', 'name': 'win2', 'title': "Title 2", 'url': 'http://localhost/page2.html'},
            {'name': 'win3', 'title': "Title 3", 'url': 'http://localhost/page3.html'})

        self.assertEqual(manager.get_window_ids(browser), ['win1', 'win2', 'undefined'])
    ...
    def _make_mock_browser(self, *window_specs):
        ...
        browser.get_current_window_info = lambda: (
            browser.current_window.handle, browser.current_window.id, browser.current_window.name, browser.current_window.title, browser.current_window.url)
