...
class ElementFinderTests(unittest.TestCase):
    def test_find_with_invalid_prefix(self):
        ...
        browser = mock()
        assert_raises_with_msg(ValueError, "Element locator with prefix 'something' is not supported", finder.find, browser, "something=test1")
        assert_raises_with_msg(ValueError, "Element locator with prefix ' by ID ' is not supported", finder.find, browser, " by ID =test1")

    def test_find_with_null_browser(self):
        finder = ElementFinder()
        self.assertRaises(AssertionError, finder.find, None, "id=test1")

    def test_find_with_null_locator(self):
        finder = ElementFinder()
        browser = mock()
        self.assertRaises(AssertionError, finder.find, browser, None)
    ...
    def test_find_with_explicit_default_strategy_and_equals(self):
        ...
        verify(browser).find_elements_by_xpath(
            "//a[(@id='page.do?foo=bar' or @name='page.do?foo=bar' or @href='page.do?foo=bar' or " + "normalize-space(descendant-or-self::text())='page.do?foo=bar' or " + "@href='http://localhost/page.do?foo=bar')]")
    ...
    def test_find_with_img(self):
        ...
        verify(browser).find_elements_by_xpath("//img[(@id='test1' or @name='test1' or @src='test1' or @alt='test1' or @src='http://localhost/test1')]")
    ...
    def test_find_with_input(self):
        ...
        verify(browser).find_elements_by_xpath("//input[(@id='test1' or @name='test1' or @value='test1' or @src='test1' or @src='http://localhost/test1')]")
    ...
    def test_find_with_select(self):
        ...
        verify(browser).find_elements_by_xpath("//select[(@id='test1' or @name='test1')]")
    ...
    def test_find_by_dom(self):
        ...
        elements = self._make_mock_elements('div', 'a', 'span', 'a')
        when(browser).execute_script("return document.getElementsByTagName('a');").thenReturn([elements[1], elements[3]])
    ...
    def test_find_returns_bad_values(self):
        ...
        for bad_value in (None, {'': None}):
            for func_name in (
            'find_elements_by_id', 'find_elements_by_name', 'find_elements_by_xpath', 'find_elements_by_link_text', 'find_elements_by_css_selector',
            'find_elements_by_tag_name'):
                when_find_func = getattr(when(browser), func_name)
                when_find_func(any()).thenReturn(bad_value)
            for locator in ("identifier=it", "id=it", "name=it", "xpath=//div", "link=it", "css=div.it", "tag=div", "default"):
