...
ROBOT_ARGS = [
    "--doc",
    "SeleniumSPacceptanceSPtestsSPwithSP%(browser)s",
    "--outputdir",
    "%(outdir)s",
    "--variable",
    "browser:%(browser)s",
    ...
    "none",
    "--log",
    "none",  # '--suite', 'Acceptance.Keywords.Textfields',
    "--loglevel",
    "DEBUG",
    "--pythonpath",
    "%(pythonpath)s",
    "--noncritical",
    "known_issue_-_%(pyVersion)s",
    "--noncritical",
    "known_issue_-_%(browser)s",
]
REBOT_ARGS = [
    "--outputdir",
    "%(outdir)s",
    "--name",
    "%(browser)sSPAcceptanceSPTests",
    "--escape",
    "space:SP",
    "--critical",
    "regression",
...
def start_http_server():
    ...
    Popen(
        ["python", env.HTTP_SERVER_FILE, "start"],
...
def execute_tests(runner, args):
    ...
    command = (
        [runner]
        + [arg % ARG_VALUES for arg in ROBOT_ARGS]
        + args
...
def process_output(args):
    ...
    if _has_robot_27():
        call(
            [
                "python",
                os.path.join(env.RESOURCES_DIR, "statuschecker.py"),
    ...
    rebot_cmd = (
        [rebot]
        + [arg % ARG_VALUES for arg in REBOT_ARGS]
        + args
