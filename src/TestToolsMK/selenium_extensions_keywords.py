...
@library
class ExtendedSeleniumLibrary(object):
    WIDTH_DEFAULT = "1366"
    HEIGHT_DEFAULT = "768"
    ...
    @keyword
    def set_browser_size_and_position(
        self, width=WIDTH_DEFAULT, height=HEIGHT_DEFAULT, x=0, y=0
    ):
    ...
    @keyword
    def click_element_extended(
        self, locator, modifier=False, action_chain=False, timeout=None, error_msg=None
    ):
        """
    ...
    @keyword
    def double_click_element_extended(
        self, locator, modifier=False, action_chain=False, timeout=None, error=None
    ):
        """
    ...
    def element_attribute_should_be(
        self, locator, attribute, attribute_value_expected, msg=None, values=True
    ):
        actual_value = sl().get_element_attribute(locator + "@" + attribute)
        # noinspection PyProtectedMember
        actual_value, attribute_value_expected = [
            bi()._convert_to_string(i) for i in (actual_value, attribute_value_expected)
        ]
    ...
    @keyword
    def create_download_dir_profile_for_firefox(
        self, path_to_download, mime_types_file=None, *extensions_files
    ):
        """
        Example use
    ...
    def create_download_dir_capabilities_for_chrome(
        self, path_to_download, **extensions_files
    ):
        """
        Example use
        ...
        logger.info(
            "Chrome Capabilities set download dir '" + path_to_download_check + "'"
        )
