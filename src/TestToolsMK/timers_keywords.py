...
class TimerKeywords(Time, Date):
    ...
    def timer_start(self, timer_name="Global"):
        ...
        if timer_name in self.timers_dict:
            error = 'Timer with name "{timer_name}" already stated at {str(self.timers_dict[timer_name])}'
            logger.warn(error)
        self.timers_dict[timer_name] = current_time
        msg = f'Timer with name "{timer_name}" stated at {str(self.timers_dict[timer_name])}'
        logger.info(msg)

    @keyword
    def timer_stop(
        self, timer_name="Global", result_format="number", exclude_millis="True"
    ):
        current_time = get_current_time_for_timers()
        if timer_name not in self.timers_dict:
            message = f"Time '{timer_name}' is not started."
            raise AssertionError(message)
        else:
            delta = Time(current_time - self.timers_dict[timer_name]).convert(
    ...
    def timer_log(
        ...
        if timer_name not in self.timers_dict:
            ...
            msg = 'Timer "%s" current results is %s' % (
                timer_name,
                Time(current_time - self.timers_dict[timer_name]).convert(
    ...
    def timer_should_be_lesser_then(self, expected_time, timer_name="Global"):
        if timer_name not in self.timers_dict:
            ...
            delta = get_current_time_for_timers() - self.timers_dict[timer_name]
            ...
            if delta > expected_time:
                message = (
                    "Timer '%s' is above expected time. Actual time %s > %s expected time "
    ...
    def timer_should_be_greater_then(self, expected_time, timer_name="Global"):
        if timer_name not in self.timers_dict:
            ...
            expected_time = Time._convert_time_to_seconds(self, expected_time)
            if delta < expected_time:
                message = f"Timer '{timer_name}' is below expected time. Actual time {delta} < {expected_time} expected time "
                raise AssertionError(message)
            return delta
