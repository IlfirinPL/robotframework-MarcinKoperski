...
class TimerKeywords(Time, Date):
    ...
    def timer_start(self, timer_name="Global"):
        ...
        if timer_name in self.TIMERS_DICTIONARY:
            error = 'Timer with name "%s" already stated at %s' % (
                timer_name,
                self.TIMERS_DICTIONARY[timer_name].__str__(),
            )
            logger.warn(error)
        self.TIMERS_DICTIONARY[timer_name] = current_time
        msg = 'Timer with name "%s" stated at %s' % (
            timer_name,
            self.TIMERS_DICTIONARY[timer_name].__str__(),
        )
        logger.info(msg)
    ...
    def timer_stop(
        ...
        if timer_name not in self.TIMERS_DICTIONARY:
            ...
            delta = Time(current_time - self.TIMERS_DICTIONARY[timer_name]).convert(
    ...
    def timer_log(
        ...
        if timer_name not in self.TIMERS_DICTIONARY:
            ...
            msg = 'Timer "%s" current results is %s' % (
                timer_name,
                Time(current_time - self.TIMERS_DICTIONARY[timer_name]).convert(
    ...
    def timer_should_be_greater_then(self, expected_time, timer_name="Global"):
        if timer_name not in self.TIMERS_DICTIONARY:
            ...
            if delta < expected_time:
                message = (
                    "Timer '%s' is below expected time. Actual time %s < %s expected time "
                    % (timer_name, delta, expected_time)
                )
                raise AssertionError(message)
            return delta
